...
modifies test files, touches core code (`engine/`, `stages/`, `connectors/`) without adding tests, or
is test-only. Produces an **internal** quality assessment — do not paste the grades into the PR;
convert only the worst 1-2 findings into inline comments (they count against the comment budget).
...
Same detection as Step 6 (perf verification) — cross-referenced from
[perf-verification.md](perf-verification.md). Determine available GPU/accelerator and installed model
assets before deciding what can run.
