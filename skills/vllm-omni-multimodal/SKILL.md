...
**Update (2026-04-13):** Qwen3-Omni is now compatible with the v2 model runner (vllm 0.19). Uses native `launch_core_engines` instead of custom spawning. `add_streaming_update` API removed; audio output tensors are now explicitly converted to float. CUDAGraph supports tuple-returning thinker model. Fixed in #2522.
